...
await ev(() => { const g = window.__glim.game; g.s.storehouse.push({ id: 'wood', n: 12 }, { id: 'egg', n: 3 }); g.give('seed_melon', 3, true); g.greenhousePlant(0, 'seed_melon'); g.shipSlot(g.s.inv.findIndex(s => s?.id === 'egg')); g.p.fastTravel = g.world.glimmers.filter(x => x.kind === 'ring').map(x => x.id); });
await open('gift', { job: 'cook' }); await shot('23-gift', 150);
await open('ship', {}); await shot('24-ship', 150);
await open('storehouse', {}); await shot('25-storehouse', 150);
await open('greenhouse', {}); await shot('26-greenhouse', 150);
await open('travel', { from: 0 }); await shot('27-travel', 150);
await ev(() => { const g = window.__glim.game, w = g.world; for (let i = w.cy * w.W + w.cx + 9; i < w.W * w.H; i++) if (w.region[i] === 0 && !w.obj[i] && !g.bmap.has(i) && w.road[i] !== 2) { g.give('chest_item', 1, true); g.give('jar', 1, true); g.placeObj(i, 'chest_item'); g.s.stations['p' + i] = { kind: 'chest', items: new Array(24).fill(null) }; g.placeObj(i + 1, 'jar'); g.s.stations['p' + (i + 1)] = { kind: 'jar', queue: [] }; window.__chestKey = 'p' + i; window.__jarKey = 'p' + (i + 1); break; } });
await ev(() => window.__glim.ui.panels.open('chest', { key: window.__chestKey })); await shot('28-chest', 150);
await ev(() => window.__glim.ui.panels.open('station', { key: window.__jarKey, station: 'jar' })); await shot('29-jar', 150);
await open('menu', { tab: 'me' }); await shot('30-you', 150);
await open('menu', { tab: 'recipes' }); await shot('31-recipes', 150);
await open('menu', { tab: 'journal' }); await shot('32-journal', 150);
